...
def bootstrap(
    ...
    """Return statistic, one-sided bootstrap support, percentile CI, and N."""
    f, o = _valid(forecast, observed, min_samples)
    if f is None:
...
def _score_matrix(
    ...
    for metric in ["ACC", "CENTERED_ACC", "MSSS"]:
        ...
        if fdr_scope == "region":
            ...
            raise ValueError("fdr_scope must be 'region' or 'global'.")
...
def plot_monthly_skill(skill_ds, configuration="uncorrected", region="EAMS", savepath=None, exclude_lead0=True):
    ...
    for ax, values, sig, title in zip(
        ...
        [acc, ms],
        [sig_acc, sig_ms],
        [r"$\mathbf{(a)}$ Uncentered ACC", r"$\mathbf{(b)}$ MSSS"],
    ):
    ...
    colorbars = [
        fig.colorbar(meshes[0], col=(1, 1), loc="b", label="Uncentered ACC", ticklabelsize=SMALL_FONT, labelsize=SMALL_FONT, length=0.82, extendsize=0.5, pad=0.40, width=0.10),
...
def build_acc_refinement(
    ...
    with xr.open_dataset(mode1_path) as mode1_ds, xr.open_dataset(full_model_path) as full_ds:
        mode1_forecast = mode1_ds["sst_forecast_anomaly"].load()
        full_model = full_ds["sst_forecast_anomaly"].load()
        observed = full_ds["sst_observed_anomaly"].load()
